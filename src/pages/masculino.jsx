  ...
  useEffect(() => {
    const carregarProdutos = async () => {
      try {
        ...
        const apenasMasculino = dados.filter(p => p.categoria === 'Masculino');
        setProdutos(apenasMasculino);
      } catch (error) {
        console.error("Erro ao integrar com o Banco de Dados no setor masculino:", error);
      } finally {
        setCarregando(false);
      }
    };
  ...
  return (
    <div className="pagina-masculino">
      ...
      <header className="masculino-header">
        ...
        <p>Camisetas, calças e bermudas com o melhor do estilo contemporâneo</p>
      </header>

      {carregando && <p className="status-texto">Carregando catálogo masculino...</p>}

      {!carregando && produtos.length === 0 && (
        <p className="status-texto">Nenhum produto cadastrado na categoria Masculino.</p>
      )}
