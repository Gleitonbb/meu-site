...
function Esportes() {
  ...
  useEffect(() => {
    const carregarProdutos = async () => {
      try {
        ...
        const apenasEsportes = dados.filter(p => p.categoria === 'Esportes');
        setProdutos(apenasEsportes);
      } catch (error) {
        console.error("Erro ao integrar com o Banco de Dados no setor esportivo:", error);
      } finally {
        setCarregando(false);
      }
    };
  ...
  const confirmarPagamentoFinal = async (e) => {
    ...
    try {
      ...
      console.error("Erro na integração do pagamento esportes:", error);
  ...
  return (
    <div className="pagina-esportes">
      ...
      <header className="esportes-header">
        <h2>Artigos Esportivos Premium</h2>
        <p>Alta performance, conforto e tecnologia para elevar o nível dos seus treinos</p>
      </header>

      {carregando && <p className="status-texto">Carregando catálogo esportivo...</p>}

      {!carregando && produtos.length === 0 && (
        <p className="status-texto">Nenhum produto cadastrado na categoria Esportes.</p>
      )}
            ...
            <div className="modal-layout-flex">
              ...
              <div className="carrossel-container">
                ...
                {produtoSelecionado.imagens && produtoSelecionado.imagens.length > 0 ? (
                  <img src={produtoSelecionado.imagens[fotoAtual]} alt="Foto do produto" className="imagem-carrossel" />
                ) : (
              ...
              <div className="modal-detalhes-compra">
                ...
                <p className="modal-descricao">
                  {produtoSelecionado.descricao || "Produto de alta qualidade, caimento perfeito e costuras reforçadas projetadas para rotinas de alta intensidade."}
                </p>
