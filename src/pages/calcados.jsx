...
function Calcados() {
  ...
  useEffect(() => {
    const carregarProdutos = async () => {
      try {
        ...
        const apenasCalcados = dados.filter(p => p.categoria === 'Calçados');
        setProdutos(apenasCalcados);
      } catch (error) {
        console.error("Erro ao integrar com o Banco de Dados no setor de calçados:", error);
      } finally {
        setCarregando(false);
      }
    };
      ...
      if (itemExistente) {
        return carrinhoAtual.map(item =>
          (item.id === produto.id && item.tamanhoEscolhido === tamanhoSelecionado && item.corEscolhida === corSelecionada)
            ? { ...item, quantidade: item.quantidade + 1 } // Mantendo a lógica de somar do masc
            : item
        );
      }
  ...
  const confirmarPagamentoFinal = async (e) => {
    ...
    try {
      ...
      console.error("Erro na integração do pagamento de calçados:", error);
  ...
  return (
    <div className="pagina-calcados">
      ...
      <header className="calcados-header">
        <h2>Calçados</h2>
        <p>Encontre o par perfeito para qualquer estilo ou terreno com o máximo de conforto</p>
      </header>

      {carregando && <p className="status-texto">Carregando catálogo de calçados...</p>}

      {!carregando && produtos.length === 0 && (
        <p className="status-texto">Nenhum produto cadastrado na categoria Calçados.</p>
      )}
            ...
            <div className="modal-layout-flex">
              ...
              <div className="carrossel-container">
                ...
                {produtoSelecionado.imagens && produtoSelecionado.imagens.length > 0 ? (
                  <img src={produtoSelecionado.imagens[fotoAtual]} alt="Foto do calçado" className="imagem-carrossel" />
                ) : (
              ...
              <div className="modal-detalhes-compra">
                ...
                <p className="modal-descricao">
                  {produtoSelecionado.descricao || "Desenvolvido com materiais de alta durabilidade e tecnologia de amortecimento ergonômico. Conforto garantido a cada passo."}
                </p>
